plot.linreg <- function(obj) {
  ...
  resid_means <- stats::aggregate(df$resid, list(df$pred), FUN=mean)
  names(resid_means) <- c("pred","resid")
  resid_medians <- stats::aggregate(df$resid, list(df$pred), FUN=median)
  names(resid_medians) <- c("pred","resid")

  resid_std_abs_sqrt_means <- stats::aggregate(df$resid_std_abs_sqrt, 
                                               list(df$pred), FUN=mean)
  names(resid_std_abs_sqrt_means) <- c("pred","resid_std_abs_sqrt")
  resid_std_abs_sqrt_medians <- stats::aggregate(df$resid_std_abs_sqrt, 
                                                 list(df$pred), FUN=median)
  ...
  pl1 <-
    ...
    ggplot2::geom_line(data=resid_means, ggplot2::aes(color="mean")) +
    ggplot2::geom_line(data=resid_medians, ggplot2::aes(color="median")) +
  ...
  #- Plot 2
  pl2 <- ggplot2::ggplot(data = df) + 
    ggplot2::aes(x=pred, y=resid_std_abs_sqrt) +
    ggplot2::geom_point(shape = 1) +
    ...
    ggplot2::geom_line(data=resid_std_abs_sqrt_medians, ggplot2::aes(color="median")) +
    ggplot2::scale_color_manual(name = "", values = c("mean" = "red",
            "median" = "darkblue")) +
